...
class TestCodexAuxiliaryAdapter:
    def test_forwards_timeout_to_responses_stream(self):
        """Codex-backed auxiliary calls must not ignore task timeouts.

        Context compression uses call_llm(task="compression"), which passes
        auxiliary.compression.timeout through chat.completions.create(). The
        Codex adapter translates that to responses.stream(); dropping timeout
        here makes the CLI look dead during slow compression.
        """
    ...
    def test_enforces_total_timeout_while_stream_keeps_emitting_events(self):
        """The SDK timeout is not enough for SSE streams that stay alive.

        Compression needs a total wall-clock deadline. Otherwise a stream that
        emits keepalive/progress events can run well past
        auxiliary.compression.timeout while the CLI appears stuck.
        """
        import time
