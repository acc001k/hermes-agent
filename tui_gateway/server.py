...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # pending_title is normally applied post-first-message (see
            # cli.exec handler). If a row already exists by the time the
            # deferred agent is ready, apply/clear it here too so duplicate-title
            # ValueError does not leave a stale pending title in memory.
            current["agent"] = agent
            pending_title = current.get("pending_title")
            if pending_title:
                try:
                    db = _get_db()
                    if db is not None and db.set_session_title(key, pending_title):
